...
**As of** : 2026-05-03 G5-bis pilot (richer head ported to E-SNN, 40 cells Option B, ~16 min M1 Max) — own-substrate g_h7a=0.1043 fail-to-reject (Welch p=0.4052 below H7B threshold 0.5), cross-substrate vs G4-ter MLX all arms reject H0 (g_mlx_minus_esnn ∈ [3.23, 4.20]) ; **H7-B classification = MLX-only artefact**, G4-ter +2.77 effect does NOT transfer to E-SNN richer head ; DR-3 evidence revised (axiom-test-level preserved, empirical effect-size transferability refuted) ; EC stays PARTIAL per pre-reg §6, FC stays C-v0.12.0. Prior : G4-quinto (H5-C CONFIRMED, RECOMBINE-empty universalises across FMNIST + CIFAR-CNN), G4-quater (H4-C CONFIRMED on FMNIST 3-layer).
