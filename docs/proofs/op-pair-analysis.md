...
| **downscale** (B) | ≠ | ⇋ commutative, NOT idempotent | ≠ | ≠ |
| **restructure** (D) | ≠ | ≠ | ≠ (topology-dependent) | ≠ |
| **recombine** (C) | ≠ | ≠ | ≠ | ≠ (sampling non-deterministic) |

**Legend** :
- ≡ : idempotent (op ∘ op = op)
- ⇋ : commutative but not idempotent (op_1 ∘ op_2 = op_2 ∘ op_1, but
  op ∘ op ≠ op)
- ≠ : non-commutative
...
- **NOT idempotent**: applying shrink_f twice gives weights × f²,
  not weights × f. So `downscale_f ∘ downscale_f ≠ downscale_f`.
- **Commutative**: for any factors f and g,
  `downscale_g ∘ downscale_f = downscale_f ∘ downscale_g` because
  scalar multiplication is commutative (weights × f × g =
  weights × g × f). Composition with self is just a special case.
...
- **4 self-pairs** : 1 idempotent (replay with same slice),
  1 commutative-but-not-idempotent (downscale: shrink² ≠ shrink),
  2 non-commutative (restructure, recombine).
