"""5-layer hierarchical MLX MLP classifier for G4-quinto Step 1.

Port of ``experiments.g4_quater_test.deeper_classifier.
G4HierarchicalDeeperClassifier`` retargeted at Split-CIFAR-10 ;
the architecture is identical but the default sizing widens to
match the larger input :

    in_dim = 3072 (32 x 32 x 3 flattened)
    hidden = (256, 128, 64, 32)

RESTRUCTURE site = ``_l3 : Linear(h2, h3)`` (middle layer).
RECOMBINE sampling site = ``latent()`` -> hidden_3 activations
(dim 64). DR-0 accountability is provided by the dream-episode
wrapper — this module exposes only the model + train/eval
primitives.

Reference :
    docs/specs/2026-04-17-dreamofkiki-framework-C-design.md sec 3.1
    docs/osf-prereg-g4-quinto-pilot.md sec 2 (H5-A)
...
@dataclass
class G4HierarchicalCIFARClassifier:
    """Deeper (5-layer) MLP classifier for Split-CIFAR-10 2-class tasks.
    ...
    def __post_init__(self) -> None:
        if len(self.hidden) != 4:
            raise ValueError(
                "G4HierarchicalCIFARClassifier expects 4 hidden "
